...
export const exportToGithub = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const internalKey = process.env.NEONSCRIPT_CONVEX_INTERNAL_KEY;
    if (!internalKey) {
      throw new NonRetriableError("NEONSCRIPT_CONVEX_INTERNAL_KEY is not configured");
    };
    ...
    const { data: repo } = await step.run("create-repo", async () => {
      return await octokit.rest.repos.createForAuthenticatedUser({
        name: repoName,
        description: description || `Exported from NeonScript on ${new Date().toLocaleDateString()}`,
    ...
    const { data: commit } = await step.run("create-commit", async () => {
      return await octokit.rest.git.createCommit({
        owner: user.login,
        repo: repoName,
        message: "Initial commit from NeonScript ",
        tree: tree.sha,
